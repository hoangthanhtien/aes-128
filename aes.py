...
def bytes2matrix(text):
    """ Convert array 16-byte thành ma trận 4x4 """
    return [list(text[i:i+4]) for i in range(0, len(text), 4)]

def matrix2bytes(matrix):
    """ Convert ma trận 4x4 matrix thành array 16-byte """
    return bytes(sum(matrix, []))

def xor_bytes(a, b):
    """ Trả về một mảng byte với các phần tử xor """
    return bytes(i^j for i, j in zip(a, b))

def inc_bytes(a):
    """ Tăng các phần tử byte trong mảng lên một đơn vị """
    out = list(a)
...
class AES:
    ...
    def __init__(self, master_key):
        """
        Khởi tạo object với key đã cho 
        """
    ...
    def _expand_key(self, master_key):
        """
        Mở rộng và trả về một list các ma trận khóa ứng với master_key
        """
        # Khởi tạo khóa vòng
        ...
        # Nhóm các khóa của words thành ma trận 4x4 
        return [key_columns[4*i : 4*(i+1)] for i in range(len(key_columns) // 4)]

    def encrypt_block(self, plaintext):
        """
        Mã hóa một block đơn 16 byte
        """
    ...
    def decrypt_cfb(self, ciphertext, iv):
        """
        Giải mã ciphertext với vector được khởi tạo ban đầu
        """
        assert len(iv) == 16
